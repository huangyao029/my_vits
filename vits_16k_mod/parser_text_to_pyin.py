...
def tone_to_digit(pyin, issplit):
...
def get_pyin(text, tone=False, issplit = True):
    ...
    while i < len(text):
            ...
            for item in phrase[t]:
                ...
                if text[i:i+len(pz)] == pz:
                    for j in range(len(pz)):
                        res.append(tone_to_digit(py[j], issplit))
                    i += len(pz)
                    flag = 1
                    break 
            if flag == 1:
        ...
        if t in hz:
            ...
            res.append(tone_to_digit(hz[t][0], issplit))
        else:
            res.append(t)
...
if __name__ == '__main__':
    ...
    text = '一个银行家正在路上行走。'
    text = '一个数学家正在数钱。'
    text = '一个数学家正在数数，他只是说说而已。'
    text = '一个银行家说这个行当不太行了。'
    pyin, txt = get_pyin(text, False, True)
    print(pyin, txt)
    
    # text = '如今#3，中轴#1九门#3独缺#2地安门#4。'
